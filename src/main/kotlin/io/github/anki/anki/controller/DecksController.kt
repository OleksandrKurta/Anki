...
class DecksController(
    ...
    fun createDeck(
        ...
        service.createNewDeck(request.toDeck(securityService.getUserIdFromAuthHeader(header)))
    ...
    fun getDecks(@RequestHeader header: HttpHeaders): Mono<List<DeckDtoResponse>> =
        service.getDecks(securityService.getUserIdFromAuthHeader(header))
    ...
    fun patchDeck(
        ...
        service.updateDeck(
            request.toDeck(deckId = deckId, userId = securityService.getUserIdFromAuthHeader(header)),
        )
    ...
    fun deleteDeck(@PathVariable deckId: String, @RequestHeader header: HttpHeaders): Mono<Void> =
        service.deleteDeck(deckId, securityService.getUserIdFromAuthHeader(header))
